...
real(kind=kind(1.0d0)), dimension(1000000)::kx, ky, x1, x2, x3, x4, x5, x6, x7, x8
integer::i, fu, fi, ios
...
real(kind=kind(1.0d0)), parameter::D=0.2d0
real(kind=kind(1.0d0)), parameter::j1=0.2d0
real(kind=kind(1.0d0)), parameter::j2=0.4d0
real(kind=kind(1.0d0)), parameter::j3=0.6d0
...
open(newunit=fi, file='chevron2.txt', status='replace', action='readwrite', iostat=ios)
...
!4.2 Condutividade Hall térmica
soma2=(((x7(1)+x8(1))*berry(kx(1),ky(1))+(x7(1000000)+x8(1000000))*berry(kx(1000000),ky(1000000)))/2.0d0)*(pi**2/1000000d0)
do i=2, 999999, 1
soma2=soma2+(x7(i)+x8(i))*berry(kx(i),ky(i))*(pi**2/1000000d0)
end do
halltermica=(-1.0d0)*soma2*T1/(pi**2)
write(*,*) "O valor do condutividade Hall térmica usando uma malha quadrada de 1000x1000 pontos a", T1, "K é", halltermica

!5.Exportando os dados para um arquivo externo
do i=1, 1000000, 1
write(unit=fi, fmt=*) kx(i), ky(i), autovalor1(kx(i), ky(i)), autovalor2(kx(i), ky(i)), x3(i), x4(i), x5(i), x6(i), x7(i), x8(i)
end do

close(unit=fu, status="keep", iostat=ios)
close(unit=fi, status="keep", iostat=ios)

end program chevron2
