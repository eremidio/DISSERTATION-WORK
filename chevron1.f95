...
open(newunit=fi, file='chevron1.txt', status='replace', action='readwrite', iostat=ios)
...
do i=1, 1000000, 1
write(unit=fi, fmt=*) kx(i), ky(i), autovalor1(kx(i), ky(i)), autovalor2(kx(i), ky(i)), x3(i), x4(i), berry(kx(i), ky(i))
end do

close(unit=fu, status="keep", iostat=ios)
close(unit=fi, status="keep", iostat=ios)

end program chevron1
